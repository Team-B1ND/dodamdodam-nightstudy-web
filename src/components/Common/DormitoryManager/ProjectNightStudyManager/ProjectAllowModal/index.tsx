...
interface ProjectAllowModalProps {
  close: () => void;
  project: ProjectNightStudy;
}

const ProjectAllowModal = ({
  close,
  project,
}: ProjectAllowModalProps) => {
  const [selectedRoom, setSelectedRoom] = useState<string | null>(null);
  const {allowProject} = useManageNightStudy();
    ...
    <S.WrapModal>
      ...
      <div style={{ display: "flex", justifyContent: "end" }}>
        <DodamFilledButton
          ...
          onClick={() => {
            allowProject({ id: project.id, room: selectedRoom! }, close);
          }}
        />
      </div>
    </S.WrapModal>
...
export default ProjectAllowModal;
